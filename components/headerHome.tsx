...
export default function Header() {
  ...
  return (
    <main>
      <nav className="sticky top-0 z-50" style={{ background: 'none', position: 'absolute', width: '100%'}}>
        <div className="max-w-[90%] mx-auto py-4 flex items-center justify-between">
          <Link href={isAdmin ? '/admin/dashboard' : '/'} className="flex items-center gap-2">
            <img src={'/logo-nb-wh.png'} style={{ width: '150px' }} />
          </Link>
            ...
            {menuItems.map((item) => (
              <Link
                ...
                className={`text-md transition-colors font-semibold ${
                  isActive(item.href) ? 'text-[#fff]' : 'text-white hover:text-white-600'
                }`}
              >
                {item.name}
              </Link>
